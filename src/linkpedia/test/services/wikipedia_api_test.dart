...
void main() {
  group('fetchArticleSummary', () {
    // enough to test
    WikipediaService wikiApi =
        WikipediaService(numberOfArticles: 3, numberOfCharacters: 50);
    test('returns article summary when http response is successful', () async {
      String responseArticleSummary =
      ...
      String noLastWord =
          articleSummary.substring(0, articleSummary.lastIndexOf(' '));
    ...
    test('throws an exception when http response is not successful', () async {
      ...
      try {
        await wikiApi.fetchArticleSummary(mockHttpClient, 'Tennis');
        fail(
            'Expected exception with message "Failed to load from Wikipedia API" was not thrown');
      } on Exception catch (e) {
        expect(e, isA<Exception>());
        expect(exceptionMessageParser(e.toString()),
            equals('Failed to load from Wikipedia API'));
      }
    });
  });
  ...
  group('fetchArticles', () {
    ...
    Future<String> mockFetchArticleSummary(
        http.Client client, String articleTitle) async {
    ...
    WikipediaService wikiApi = WikipediaService(
        numberOfArticles: 3,
        numberOfCharacters: 50,
        fetchArticleSummary: mockFetchArticleSummary);
    test('returns articles when http response is successful', () async {
      final mockHttpClient = MockClient((request) async {
        final response = [
          "tennis",
          ["Tennis", "Tennis scoring system", "Tennessine"],
          ["", "", ""],
          [
      ...
      expect(articles[0].summary,
          'Tennis is a racket sport that is played either individually...');

      expect(articles[1].title, 'Tennis scoring system');
      expect(articles[1].url,
          'https://en.wikipedia.org/wiki/Tennis_scoring_system');
      expect(articles[1].summary,
          'The tennis scoring system is a standard widespread...');

      expect(articles[2].title, 'Tennessine');
      expect(articles[2].url, 'https://en.wikipedia.org/wiki/Tennessine');
      expect(articles[2].summary,
          'Tennessine is a synthetic chemical element with the...');
    });

    test('throws an exception when fetchArticleSummary throws an exception',
        () async {
      ...
      try {
        ...
        fail(
            'Expected exception with message "Failed to fetch article summary" was not thrown');
      } on Exception catch (e) {
        expect(e, isA<Exception>());
        expect(exceptionMessageParser(e.toString()),
    ...
    test('throws an exception when http response is not successful', () async {
      ...
      try {
        ...
        expect(e, isA<Exception>());
        expect(exceptionMessageParser(e.toString()),
            'Failed to load from Wikipedia API');
      }
    });
  });
}
